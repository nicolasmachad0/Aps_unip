function trocaTexto1() {
  var texto = document.getElementById("meuTexto");
  if (texto.innerHTML === "Texto inicial") {
    texto.innerHTML = "O meio ambiente nos fornece todos os recursos que precisamos para a nossa sobrevivência, e para que ele continue fazendo isso, nós precisamos cuidar do planeta que vivemos.";
  } else {
    texto.innerHTML = "O meio ambiente nos fornece todos os recursos que precisamos para a nossa sobrevivência, e para que ele continue fazendo isso, nós precisamos cuidar do planeta que vivemos.";
  }
}

function trocaTexto2() {
  var texto = document.getElementById("meuTexto");
  if (texto.innerHTML === ".") {
    texto.innerHTML = "Sustentabilidade é a busca pelo equilíbrio entre o suprimento das necessidades humanas e preservação dos recursos naturais, não comprometendo as próximas gerações.";
  } else {
    texto.innerHTML = "Sustentabilidade é a busca pelo equilíbrio entre o suprimento das necessidades humanas e preservação dos recursos naturais, não comprometendo as próximas gerações.";
  }
}

function trocaTexto3() {
  var texto = document.getElementById("meuTexto");
  if (texto.innerHTML === ".") {
    texto.innerHTML = "Refere à redução da quantidade de recursos naturais e materiais utilizados pelos indivíduos e pela sociedade como um todo. .";
  } else {
    texto.innerHTML = "Refere à redução da quantidade de recursos naturais e materiais utilizados pelos indivíduos e pela sociedade como um todo.";
  }
}

function trocaTexto4() {
  var texto = document.getElementById("meuTexto2");
  if (texto.innerHTML === "Texto inicial") {
      texto.innerHTML = "O consumismo teve origem a partir da revolução industrial. Após os processos de produção e circulação de um determinado produto ter sido agilizado, a população passou a consumir muito mais do que consumia antes.";
  } else {
      texto.innerHTML = "O consumismo teve origem a partir da revolução industrial. Após os processos de produção e circulação de um determinado produto ter sido agilizado, a população passou a consumir muito mais do que consumia antes.";
  }
}

function trocaTexto5() {
  var texto = document.getElementById("meuTexto2");
  if (texto.innerHTML === ".") {
      texto.innerHTML = "A compreensão da importância do papel da sustentabilidade no meio ambiente é fundamental e já está sendo colocada em prática pela população e grande parte das empresas, que buscam minimizar o impacto de suas ações na natureza por meio da reciclagem de seus materiais e a utilização de fontes renováveis de energia.";
  } else {
      texto.innerHTML = "A compreensão da importância do papel da sustentabilidade no meio ambiente é fundamental e já está sendo colocada em prática pela população e grande parte das empresas, que buscam minimizar o impacto de suas ações na natureza por meio da reciclagem de seus materiais e a utilização de fontes renováveis de energia.";
  }
}

function trocaTexto6() {
  var texto = document.getElementById("meuTexto2");
  if (texto.innerHTML === ".") {
      texto.innerHTML = "Em Sorocaba e região, são cidades que tendem aumentar mais e mais o consumo por conta grandes desenvolvimentos das empresas e industrias. ";
  } else {
      texto.innerHTML = "Em Sorocaba e região, são cidades que tendem aumentar mais e mais o consumo por conta grandes desenvolvimentos das empresas e industrias. ";
  }
}


function trocaTexto7() {
  var texto = document.getElementById("meuTexto3");
  if (texto.innerHTML === "Texto inicial") {
      texto.innerHTML = "Implemetada em 2020, a Sede da Secretaria do Meio Ambiente e Sustentabilidade é ponto de entrega voluntária de lixo eletrônico em Sorocaba. Todo o material é recolhido e transportado por empresas certificadas e homologadas, que fazem a reciclagem e destinação correta do lixo eletrônico.";
  } else {
      texto.innerHTML = "Implemetada em 2020, a Sede da Secretaria do Meio Ambiente e Sustentabilidade é ponto de entrega voluntária de lixo eletrônico em Sorocaba. Todo o material é recolhido e transportado por empresas certificadas e homologadas, que fazem a reciclagem e destinação correta do lixo eletrônico.";
  }
}

function trocaTexto8() {
  var texto = document.getElementById("meuTexto3");
  if (texto.innerHTML === ".") {
      texto.innerHTML = "No hospital Unimed de Sorocaba, este resultado foi percebido por meio da diminuição do consumo de energia, água e de materiais, notadamente os copos descartáveis e papel. Indiretamente, toda a sociedade é beneficiada. Por meio das ações do programa, pode-se perceber, mediante os controles de gastos, acentuadas reduções ao uso de energia, água e materiais, fato que auxilia a redução dos impactos ambientais.";
  } else {
      texto.innerHTML = "No hospital Unimed de Sorocaba, este resultado foi percebido por meio da diminuição do consumo de energia, água e de materiais, notadamente os copos descartáveis e papel. Indiretamente, toda a sociedade é beneficiada. Por meio das ações do programa, pode-se perceber, mediante os controles de gastos, acentuadas reduções ao uso de energia, água e materiais, fato que auxilia a redução dos impactos ambientais.";
  }
}

function trocaTexto9() {
  var texto = document.getElementById("meuTexto3");
  if (texto.innerHTML === ".") {
      texto.innerHTML = "Sorocaba vem desenvolvendo nos últimos anos ações de sustentabilidade com implantação de energia fotovoltaica em prédios públicos municipais. Trata-se de uma das formas mais limpas de produção de energia e que mais cresce no mundo, composta por painéis que captam a energia do sol. Além de reduzir a emissão de gases do efeito estufa e o consumo de energia, a medida sustentável ainda gera economia de recursos aos cofres públicos.";
  } else {
      texto.innerHTML = "Sorocaba vem desenvolvendo nos últimos anos ações de sustentabilidade com implantação de energia fotovoltaica em prédios públicos municipais. Trata-se de uma das formas mais limpas de produção de energia e que mais cresce no mundo, composta por painéis que captam a energia do sol. Além de reduzir a emissão de gases do efeito estufa e o consumo de energia, a medida sustentável ainda gera economia de recursos aos cofres públicos.";
  }
}